...
없으므로 실제 limit 시험에는 CCW limit를 원점으로 사용하는 Method 8을 사용했다.
현재 운전 좌표는 그 기준의 `+179.000 deg`에서 Method 10으로 정한 작업 원점이다. 3축
...
| 5 | 측정 357.350° | 양쪽 하드 리미트에서 약 5.2° | -173.786 / +173.134° |
...
4. 3축은 중심에서 Method 10을 실행하고, 5축 작업 원점 상실 시 Method 8 복구 절차 후 `+179.000 deg`에서 Method 10 실행
...
| 2 | 빔에 수직인 횡방향 직선축; EPICS +가 오른쪽 | `Pos` |
| 3 | 상하 Z축 | `Neg` |
...
실제 장비의 사용자 방향은 `+X=앞`, `+Y=오른쪽`, `+Z=위`로 확인됐다. 이 세 방향을
그대로 사용하면 왼손 좌표가 되므로 표준 오른손 회전행렬을 사용하는 고정점 계산에서는
내부적으로 `Xc=X`, `Yc=-Y`, `Zc=Z`로 변환한다. 상세 기구 모델은
